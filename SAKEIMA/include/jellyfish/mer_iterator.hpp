...
public:
  ...
  mer_iterator() : job_(0), cseq_(0), filled_(0), canonical_(false), index(0) { }
  ...
  mer_iterator& operator++() {
    //std::cout << "op1 - " << index << std::endl;
    //++index;
    while(true) {
      while(cseq_ == (*job_)->end) {
        //std::cout << " calling job_->next() " << std::endl;
        job_->next();
        //std::cout << " called job_->next(); " << std::endl;
        if(job_->is_empty()) {
          delete job_;
          job_  = 0;
          cseq_ = 0;
        ...
        cseq_   = (*job_)->start;
        //std::cout << " (*job_)->start " << (*job_)->start <<  std::endl;
        filled_ = 0;
      }
    ...
    //std::cout << " *cseq_ " << *cseq_ << std::endl;
    return *this;
  }

  mer_iterator operator++(int) {
    //std::cout << "op2 - " << index << std::endl;
