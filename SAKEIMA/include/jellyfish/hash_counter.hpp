...
protected:
  ...
  bool double_size(bool serial_thread) {
    //std::cout << " call to double_size " << std::endl;
    if(serial_thread) {// Allocate new array for size doubling
      try {
        if(ary_->key_len() >= sizeof(size_t) * 8 || ary_->size() < ((size_t)1 << ary_->key_len())) {
          ...
          new_ary_   = new array(ary_->size() * 1.5, ary_->key_len(), ary_->val_len(),
    ...
    //std::cout << " call to double_size end with size " << ary_->size() << std::endl;
