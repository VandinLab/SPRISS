...
times_file = open("SAKEIMA_time.txt",'w')
for run in range(n_runs):
	of = open("sakeima_results_run"+str(run)+".txt",'r')
...
distances_file = open("SAKEIMA_distances_approx.txt",'w')
for run in range(n_runs):
	of = open("output_approx_distances_run"+str(run)+"_withsakeima.txt",'r')
	for t,theta in enumerate(thetas):
		line = of.readline()
		z = 0
		for i in range(len(datasets)):
...
for i in range(len(datasets)):
	...
	for j in range(len(thetas)):
		...
		for k in range(3):
			rates[i][j].append([])
quality_file = open("SAKEIMA_quality.txt",'w')
for run in range(n_runs):
